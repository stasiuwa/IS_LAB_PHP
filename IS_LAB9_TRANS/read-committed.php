...
// Change the default isolation level (default is REPEATABLE READ)
 ...
 // To test how to isolation level works you should invoke update-adam.php from the second application at this moment,
 // that the database state will be changed
 sleep(5);
 // You should see different result of the query even though both queries are executed during one transaciton.
// This is due to the altered level of isolation that allows committed changes to be read from other transactions
 echo "After sleep<br>";
$sql = "SELECT actor_id, first_name, last_name FROM actor WHERE first_name = 'ADAM'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
